from tkinter import *
root=Tk()
root.title("VapidTech Calculator 1.1")
...
def button_add():
       num1=int(e.get())                   #number entered before using operand is stored
...
def button_equal():
       num2=int(e.get())                   #number entered after using operand is stored
       e.delete(0,END)
...
mybutton_equal=Button(root,text="=",command=button_equal,pady=20,width=40).grid(row=5,column=1,columnspan=2)
mybutton_clear=Button(root,text="Clear",command=button_clear,pady=20,width=20).grid(row=5,column=0,columnspan=1)
mybutton_add=Button(root,text="+",command=button_add,pady=20,width=40).grid(row=4,column=1,columnspan=2)
